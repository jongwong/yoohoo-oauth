...
@Aspect
@Component
public class AutoCreatedAspect {

    @Before("execution(* cn.jongwong.server..*(.., @cn.jongwong.server.common.AutoCreatedField (*)))")
                ...
                if (data != null) {
                    ...
                    try {
                        ...
                        for (Field field : fields) {
                            ...
                            if ("createdAt".equals(field.getName()) && field.get(data) == null) {
                                field.set(data, LocalDateTime.now());  // 填充时间戳
                            }
                            if ("createdBy".equals(field.getName()) && field.get(data) == null) {
                                field.set(data, user.getId());  // 填充用户名
                            }
                            if ("createdByName".equals(field.getName()) && field.get(data) == null) {
