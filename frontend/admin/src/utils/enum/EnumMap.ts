 ...
 */
export class EnumMap<T = any, V extends ValueEnumType<T> = ValueEnumType<T>> extends Map<V['value'], V> implements ReadonlyMap<V['value'], V> {
    ...
    list(config: EnumMapListParams<'text' | 'label', true>): (V & { label: string })[];
    ...
    list(config: EnumMapListParams<'text' | 'label', boolean> = {}): V[] {
        ...
        const {limitText, limitValue, addUnLimit, type, isOptions, excludeByValues = []} = finalConfig;

        const _list = Array.from(this.values()).filter(item => !(excludeByValues as any)?.includes(item.value));
        if (addUnLimit) {
...
export type ReadonlyEnumMap<T = any, V extends ValueEnumType<T> = ValueEnumType<T>> = Omit<EnumMap<T, V>, 'set' | 'clear' | 'delete'>;
